...
def render_diff(a_name: str, b_name: str, diff: Dict[str, Any]) -> str:
    ...
    """
    template_path = (
        Path(__file__).parent.parent.parent / "docs" / "DIFF_REPORT_TEMPLATE.md"
    )
    ...
    out = out.replace(
        "| <param_name>  | <A_value>   | <B_value>   | <major/minor/none> |", table
    )
    return out
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Diff two MATRIX threshold sets (TS_*.yml)"
    )
    ...
    parser.add_argument(
        "--out", type=str, required=True, help="Path to output DIFF_*.md"
    )
