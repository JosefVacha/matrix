...
def fill_report(report_path, data):
    ...
    text = pathlib.Path(report_path).read_text(encoding="utf-8") if pathlib.Path(report_path).exists() else ""
    def marker(name, content):
        return f"<!-- {name} -->\n{content}\n<!-- /{name} -->"
    # Fill RUN_META
    run_meta = data.get("run_meta", {})
    run_meta_str = "\n".join(f"{k}: {v}" for k, v in run_meta.items())
    text = re.sub(r"<!-- RUN_META -->([\s\S]*?)<!-- /RUN_META -->", marker("RUN_META", run_meta_str), text) if "<!-- RUN_META -->" in text else text + "\n" + marker("RUN_META", run_meta_str)
    # Fill PERF_PROXY
    perf = data.get("perf", {})
    perf_str = "\n".join(f"{k}: {v}" for k, v in perf.items())
    text = re.sub(r"<!-- PERF_PROXY -->([\s\S]*?)<!-- /PERF_PROXY -->", marker("PERF_PROXY", perf_str), text) if "<!-- PERF_PROXY -->" in text else text + "\n" + marker("PERF_PROXY", perf_str)
    # Fill SIGNALS
    signals = data.get("signals", {})
    signals_str = "\n".join(f"{k}: {v}" for k, v in signals.items())
    text = re.sub(r"<!-- SIGNALS -->([\s\S]*?)<!-- /SIGNALS -->", marker("SIGNALS", signals_str), text) if "<!-- SIGNALS -->" in text else text + "\n" + marker("SIGNALS", signals_str)
...
def main():
    parser = argparse.ArgumentParser(description="Ingest Freqtrade report and fill markers.")
    ...
    if not data:
        ...
        if rows:
            ...
            agg = {k: sum(float(r.get(k,0)) for r in rows)/len(rows) for k in ("avg_trade_return","win_rate","max_drawdown") if k in rows[0]}
            data = {"perf": agg}
    if not data:
