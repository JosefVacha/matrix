            ...
            if typ:
                if isinstance(typ, list):
                    if not any(isinstance(meta[k], _type_py(t)) for t in typ):
                        errors.append(f"Key {k} type mismatch: {type(meta[k])} not in {typ}")
                else:
                    if not isinstance(meta[k], _type_py(typ)):
                        errors.append(f"Key {k} type mismatch: {type(meta[k])} != {typ}")
            # Enum check
            if "enum" in v:
...
def main():
    parser = argparse.ArgumentParser(description="Validate model registry metadata against schema")
