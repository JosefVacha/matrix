...
def check_tasks_json(auto_fix: bool = True) -> Tuple[bool, bool]:
    ...
    for t in tasks:
        if t.get('label') == 'registry-check' and t.get('type') == 'shell':
            ...
            if cmd.startswith('echo python scripts/qa/validate_model_metadata.py --file models/BASELINE_LIN_H3/metadata.json --schema docs/schemas/model_metadata.schema.json'):
                registry_ok = True
...
def todo_audit() -> str:
    ...
    # registry proof
    init1, _ = run_cmd(['python3', 'scripts/registry/init_model_tag.py', '--tag', 'BASELINE_LIN_H3'])
    init2, _ = run_cmd(['python3', 'scripts/registry/init_model_tag.py', '--tag', 'BASELINE_LIN_H3'])
    val, _ = run_cmd(['python3', 'scripts/qa/validate_model_metadata.py', '--file', 'models/BASELINE_LIN_H3/metadata.json', '--schema', 'docs/schemas/model_metadata.schema.json'])
    summary.append(f'registry_init:{init1},{init2}')
    summary.append(f'registry_validate:{val}')
    # H-consistency
    hcode, _ = run_cmd(['python3', 'scripts/qa/check_H_consistency.py', '--label-name', 'label_R_H12_pct', '--windows', '1,3,12', '--H', '12'])
...
def main():
    ...
    if not guard_ok or en_code != 0 or not registry_ok or not h_ok or not ci_ok or not model_ref_ok or not eval_ok:
        status = 'FAIL'
