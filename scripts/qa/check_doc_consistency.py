...
MUST_TERMS = [
    ...
    "hysteresis", "cooldown", "no leakage", "0/1 lists", "same length as predictions"
]

TARGETS = {
    "mapping.py": ("src/matrix/strategy/mapping.py", r"def map_predictions_to_signals\(.*?\):([\s\S]*?)def |$"),
    "adapter.py": ("src/matrix/adapter/freqtrade_strategy_adapter.py", r"""(\"\"\"[\s\S]*?\"\"\")"""),
...
def main():
    parser = argparse.ArgumentParser(description="Check doc consistency across mapping, adapter, CONTRACTS.md.")
