...
def test_cooldown():
    ...
    out = map_predictions_to_signals(
        preds, up=0.1, dn=-0.1, hysteresis=0.02, cooldown_bars=2
    )
...
def test_hysteresis():
    ...
    out = map_predictions_to_signals(
        preds, up=0.1, dn=-0.1, hysteresis=0.1, cooldown_bars=1
    )
...
def test_output_shape():
    ...
    out = map_predictions_to_signals(
        preds, up=0.1, dn=-0.1, hysteresis=0.02, cooldown_bars=1
    )
    n = len(preds)
    for k in out:
        assert len(out[k]) == n
