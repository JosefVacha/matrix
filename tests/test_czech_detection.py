...
def run_guard_json():
    p = subprocess.run(["python3", "scripts/qa/check_copilot_guardrails.py", "--json"], capture_output=True, text=True)
    try:
...
def test_negative_detection_with_english():
    ...
    try:
        ...
        modified = "\n".join([ln for ln in orig.splitlines() if 'češt' not in ln and 'czech' not in ln.lower()])
